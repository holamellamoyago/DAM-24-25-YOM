...
go
IF EXISTS (SELECT * FROM SYS.tables WHERE name = 'JEFE' AND SCHEMA_ID= SCHEMA_ID('GRUPOS'))
	PRINT 'EXISTE'
...
CREATE TABLE ORGANIZACIONES.ORGANIZACIONES(
	...
	CONSTRAINT FK_ORGANIZACIONES	FOREIGN KEY	(CODIGO) REFERENCES ORGANIZACIONES.ORGANIZACIONES
		--ON DELETE NO ACTION ON UPDATE CASCADE,
...
CREATE TABLE ORGANIZACIONES.MEDIA(
	...
	ORGANIZACION	TINYINT,
	FEHCA_INICIO	DATE,
	FECHA_FIN	DATE CONSTRAINT DF_FECHA_FIN DEFAULT  DATEADD(MM,2, GETDATE()),
	T_AYUDA		VARCHAR,
...
ALTER TABLE TRAFICANTES.ARMA
	ADD CAPACIDADDESTRUCTIVA INT;

GO
	
ALTER TABLE TRAFICANTES.ARMA
	WITH CHECK ADD	CONSTRAINT CK_CAPACIDAD CHECK(
			(CAPACIDADDESTRUCTIVA >= 100)
		)
...
ALTER SCHEMA ARMAS TRANSFER TRAFICANTES.ARMA;
--INSERT INTO TRAFICANTES.ARMA (CODIGO, INDICADOR, CAPACIDADDESTRUCTIVA) VALUES (1,12, 1);

GO

ALTER TABLE ARMAS.ARMA 
	ADD TIPOARMA SMALLINT
...
CREATE TABLE TIPOARMA(
	...
	CONSTRAINT PK_TIPOARMA PRIMARY KEY (CODIGO),
	
)

GO

ALTER SCHEMA ARMAS TRANSFER DBO.TIPOARMA

GO

ALTER TABLE ARMAS.ARMA
		ADD CONSTRAINT FK_TIPO_ARMA FOREIGN KEY (TIPOARMA) REFERENCES ARMAS.TIPOARMA(CODIGO),
			CONSTRAINT U_TIPOARMA UNIQUE (TIPOARMA)

GO
	
ALTER TABLE ARMAS.ARMA
	ADD ALCANCE SMALLINT
GO
	
ALTER TABLE ARMAS.ARMA
	ADD NOMBRE_ALCANCE VARCHAR(25)
	
GO
	
ALTER TABLE ARMAS.ARMA
	ADD CONSTRAINT CK_NOMBRE_ALCANCE CHECK(
		(NOMBRE_ALCANCE IN ('METROS', 'KILOMETROS', 'MILIMETROS'))
	)

GO

INSERT INTO ARMAS.ARMA(CODIGO, INDICADOR, CAPACIDADDESTRUCTIVA,ALCANCE, NOMBRE_ALCANCE)
	VALUES (1,1, 100, 1, 'METROS')

EXEC sp_help 'ARMAS.ARMA'
--SELECT * FROM TRAFICANTES.ARMA
