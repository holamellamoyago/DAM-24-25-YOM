...
--69
SELECT V.NOMBREZONA,CAST(CP.CALLE AS VARCHAR(10))  + ', ' + CAST(CP.NUMERO AS VARCHAR(10))
...
WHERE METROSCONSTRUIDOS < (
	...
	INNER JOIN VIVIENDA V2 ON V.CALLE = CP.CALLE AND CP.NUMERO=V.NUMERO
	WHERE PISCINA = 'N' AND V.NOMBREZONA = V2.NOMBREZONA
...
INNER JOIN PROPIETARIO PRO ON PRO.DNI = P.DNIPROPIETARIO 
 ...
 (
	SELECT * FROM CASAPARTICULAR CP 
				WHERE CP.DNIPROPIETARIO = PO.DNI
 ) 
 
 
 SELECT DNI, NOMBRE  FROM PROPIETARIO
WHERE DNI IN (
	SELECT DNIPROPIETARIO FROM PISO 
	GROUP BY DNIPROPIETARIO
	HAVING COUNT(*) > 1
) AND NOT EXISTS (SELECT * FROM CASAPARTICULAR WHERE DNI = DNIPROPIETARIO)